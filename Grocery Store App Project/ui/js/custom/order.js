    ...
    $.get(productListApiUrl, function (response) {
        ...
        if (response) {
            ...
            $.each(response, function (index, product) {
                ...
                discounts[product.product_id] = 0; // Initialize discounts to 0 for each product
            });
...
$("#addMoreButton").click(function () {
    ...
    $(".product-box-extra .product-discount").last().val('0'); // Initialize discount to 0 for the new row
    calculateValue();
});
...
$(document).on("change", ".product-qty, .product-discount", function (e) {
    calculateValue();
});
